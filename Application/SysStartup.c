       ...
       )
  {
    for(unsigned int cpt = 0; cpt < (__STARTUP_RUNTIME_COPYTABLE)[index].size; cpt++)
    {
      const unsigned char src_value =
        *(volatile unsigned char*)((unsigned long)((unsigned long)((__STARTUP_RUNTIME_COPYTABLE)[index].sourceAddr) + cpt));

      volatile unsigned char* dst_addr =
        (volatile unsigned char*)((unsigned long)((unsigned long)((__STARTUP_RUNTIME_COPYTABLE)[index].targetAddr) + cpt));
...
static void SysStartup_InitCtors(void)
{
  typedef void(*p_ctor_func)(void);

  for(unsigned long entry = 1; entry <= (__STARTUP_RUNTIME_CTORS)[0]; entry++)
  {
    ((p_ctor_func)((__STARTUP_RUNTIME_CTORS)[entry]))();
  }
}
